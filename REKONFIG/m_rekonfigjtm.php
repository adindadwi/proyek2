<?php
	class M_rekonfigjtm extends CI_Model
	{
		...
		function showlokfromdb()
		{
			...
			{
				$hasil=$this->db->query("SELECT * FROM kol_jtmrek WHERE ryn='".$_SESSION['rayon']."' ORDER BY id ASC")->result();
			}
			return $hasil;
		}
		...
		function simpanlkstodb($data)
		{
			...
			$jlm_ganggu=$data['jlm_ganggu'];
			$ryn=$data['ryn'];
			
			$query="INSERT into kol_jtmrek(bgr_no,gbr2_no,lks,ex_sist,penylang,jlm_ganggu,ryn,mtsu,ctivm,cgu) values('".$bgr_no."','".$gbr2_no."','".$lks."','".$ex_sist."','".$penylang."','".$jlm_ganggu."','".$ryn."','0','0','0')";
