	...
	class rehab_jar_tr extends CI_Controller
	{
       ...
       /* public function lihatrlvadb()
		...
		function saveaddlokasi(){
			...
			$data['jumlahXX']=$this->input->post('jumlahXX');
			$data['rayon2']=$_SESSION['rayon'];
			$this->load->model('m_rehab_jar_Tr');
		...
		public function addline()
        {
			...
			$data['gambar2']=$this->m_rehab_jar_tr->gambarlinefromdb($id);
		...
		function input()
		{
			...
			$data['z']=$this->input->post('z');
		  	$this->load->model('m_rehab_jar_tr');
		    $query=$this->m_rehab_jar_tr->cekinput($this->input->post('id_lokJTR'));
		    $cek=$query->row();
		     if($cek->count > 0)
		        {
